...
3. The preliminary condition to depict an event horizon in an optical fiber is that the soliton and the dispersive wave should lie in anomalous and normal dispersion regime respectively. The python script ```main_analyze_prop_const.py```  shows the group velocity matching frequency in anomalous and normal dispersion regime under the influence of positive third order dispersion. The figure below shows the group velociy matching frequency at ```\beta_2 = -1 fs^2 / μm``` and ```\beta_3 = 0.1 fs^3 / μm```.
