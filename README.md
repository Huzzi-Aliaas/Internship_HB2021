...
The python scripts provided in this repository mostly deals with the propagation scenario of an optical pulse inside an optical fiber subject to generalized nonlinear Schrödinger equation with higher order dispersion effects. It also includes efficient algorithms for the numerical solution of generalized nonlinear Schrödinger equation with global error of fourth-order. Furthermore, it also has python scripts to carry out numerical experiments for pulse-pulse interaction in presence on an optical event horizon and python scripts for generating spectrograms in order analyse propagation scenarios.


1. The python script ```solver.py``` (gnse) provided in the repository contain algorithms to solve normal and generalized nonlinear Schrödinger equations in object oriented programming. The algorithms include split step Fourier method, symmetric split step Fourier method and fourth-order Runge-kutta in interation picture method. The aim of employing different numerical schemes is to test the performance of each method by checking the global error. In order to compare the performance of different numerical schemes, the root mean square intensity error after one soliton period is plotted as a function of the step size. The graph below generated by the python script ```Quality_control.py``` (results / numExp07_quality_controll) shows that the fourth-order Runge-Kutta method clearly outperforms the other numerical schemes.

![alt text](https://github.com/omelchert/Internship_HB2021/blob/main/results/numExp07_quality_controll/Quality_control.png)

2. The python script ```main_event_horizon_v1.py``` (results / numExp02_event_horizon) provided in this repository demonstrate pulse-pulse interaction in presence of an optical event horizon. It aims to show the nonlinear interation of light in an optical fiber which mimics the physics at an event horizon of astrophysical black hole. The figure shown below is generated by ```main_event_horizon_clean.py``` (results / numExp03_cleaned_up_soliton) where a soliton in anomalous dispersion regime interacts with a dispersive wave in normal dispersion regime at a different group velocity. The dispersive wave is reflected off the soliton in time domain and in frequency domain the frequency of the dispersive wave appears to have been blue shifted. 
...
3. The preliminary condition to depict an event horizon in an optical fiber is that the soliton and the dispersive wave should lie in anomalous and normal dispersion regime respectively. The python script ```main_analyze_prop_const.py``` (results / numExp01_analyze_beta) shows the group velocity matching frequency in anomalous and normal dispersion regime under the influence of positive third order dispersion. The figure below shows the group velocity matching frequency at ```\beta_2 = -1 fs^2 / μm``` and ```\beta_3 = 0.1 fs^3 / μm```.
...
4. The python script ```main_spectrogram.py``` (results / numExp04_spectrogram) is a tool for plotting spectrogram which intuitively displays the complex field envelope instantaneous frequency vs. time. The technique used to generate spectrogram is called Frequency Resolved Optical Gating (FROG) in which a gate function gates out a piece of the complex field envelope and the spectrum of that piece is measured. The gate is then scanned through the waveform and the process is repeated for all values of the gate position. The gif shown below is a spectrogram of a white hole event horizon.

![alt text](https://github.com/omelchert/Internship_HB2021/blob/main/results/numExp04_spectrogram/spec.gif)

5. The python script ```main_sc_generation.py``` (results / numExp06_supercontinuum_generation) depicts an optical event horizon in a supercontinuum generation in which a higher order soliton is propagated inside a nonlinear medium under the influence of higher orders of dispersion in normal dispersion regime. Under such condition, soliton fission takes place and the soliton in anomalous dispersion regime in presence of an optical horizon guides the dispersive wave in normal dispersion regime with it. 
