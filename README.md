...
3. The preliminary condition to depict an event horizon in an optical fiber is that the soliton and the dispersive wave should lie in anomalous and normal dispersion regime respectively. The python script ```main_analyze_prop_const.py```  shows the group velocity matching frequency in anomalous and normal dispersion regime under the influence of positive third order dispersion. The figure below shows the group velocity matching frequency at ```\beta_2 = -1 fs^2 / μm``` and ```\beta_3 = 0.1 fs^3 / μm```.
...
5. The python script ```main_sc_generation.py``` depicts an optical event horizon in a supercontinuum generation in which a higher order soliton is propagated inside a nonlinear medium under the influence of higher orders of dispersion in normal dispersion regime. Under such condition, soliton fission takes place and the soliton in anomalous dispersion regime behaves like an event horizon and guides the dispersive wave with it. 
